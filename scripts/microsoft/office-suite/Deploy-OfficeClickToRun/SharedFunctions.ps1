...
Function Validate-UpdateSource() {
    ...
    if ($UpdateSource) {
        Write-Host "Update source: $UpdateSource"
        $mainRegPath = Get-OfficeCTRRegPath
        if ($mainRegPath) {
        ...
        Write-Host "Main cab: $mainCab"

        if (!($updateToVersion)) {
            Write-Host "Checking cab version..."
        ...
        Write-Host "UpdateToVersion: $updateToVersion."
          ...
          if ($FolderPath) {
              ...
              if (Test-Path -Path $XMLFilePath) {
                 $downloadFile = $false
                  ...
                  Write-Host "File missing $FolderPath\ofl.cab"
